...
public class SaSessionController {
    ...
    public SaResult getSession(){
        ...
        // 根据用户id  获取当前账号的session
        // AccountSession 应用场景  PC端 APP端 获取的session 是同一个，可以轻松同步数据
        SaSession AccountSession = StpUtil.getSession();
        log.info("当前账号Session AccountSession = {}", AccountSession);

        // 获取当前 Token 的 Token-Session 对象
        // Token Session 应用场景 设置超过两小时无操作就自动下线，当在手机端操作，PC端无操作时，PC端会自动下线
        SaSession tokenSession = StpUtil.getTokenSession();
        log.info("当前 tokenSession = {}", tokenSession);
